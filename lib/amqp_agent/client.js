...
exports.Client = Client = function (config) {
...
Client.prototype.getAgentHandle = function (uuid, resource, callback) {
  ...
  if (this.agentHandles[uuid]) {
    ...
    handle = this.agentHandles[uuid]
      = new AgentHandle({ connection: this.connection
...
AgentHandle.prototype.prepareAckQueue = function(callback) {
  ...
  if (!this.ackCallbacks) {
    ...
    puts("Queue has been set up already.");
    callback();
    return;
  }

  var queueName = this.resource + '.req.' + this.uuid + '.' + genId();
  puts('Awaiting ACK on queue: ' + queueName);

  var queue = this.connection.queue(queueName, { autoDelete: true }, function () {
    queue.bind('amq.topic', self.resource + '.ack' + self.clientId + '.' + self.uuid);
    queue.subscribeJSON(function (msg) {
      if (!msg.req_id) {
        puts("Ack had no request id");
        return;
      }

      // abort if we have timed out waiting for an ACK response
      if (!self.ackCallbacks[msg.req_id]) {
        puts("Didn't recognize request id on ACK as originating from this client");
        return;
      }
      ...
      if (msg.error) {
        puts('ACK indicated that an error occurred:');
        puts(msg.error);
      }
...
AgentHandle.prototype.listenForAck = function(id, callback) {
  ...
  this.ackCallbacks[id] = { callback: callback
...
AgentHandle.prototype.sendCommand = function(commandName, payload, callback) {
  ...
  puts("Publishing message to routing key: '" + routingKey + "'");

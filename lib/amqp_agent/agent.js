...
var AMQPAgent = exports.AMQPAgent = function (config) {
    var self = this;
    ...
    // Map the id's of incoming messages to the clients who sent them.
    // This is to avoid spamming all clients with ACKs they don't care
    // about.
...
AMQPAgent.prototype.configureAMQP = function (callback) {
    ...
    if (process.env['AMQP_USE_SYSTEM_CONFIG']) {
        smartdc_config.sysinfo(function (error, config) {
            self.sysinfo = config;
            // Look up and set the UUID of the machine the agent will
            // run on.
            ...
            smartdc_config.sdcConfig(function (err, config2) {
                self.sdcConfig = config2;
                var rabbitmq = config2['rabbitmq'].split(':');
                if (!rabbitmq) {
                    throw new Error(
                        'Could not find \rabbitmq\' ' +
                        'parameter from /lib/sdc/config.sh');
                }
        ...
        if (self.config.uuid || process.env['SERVER_UUID']) {
            ...
            throw new Error(
                'Could not find server uuid in config[uuid] or ' +
                'ENV[SERVER_UUID]');
        }
        setAMQPConfig(
...
AMQPAgent.prototype.handleMessage = function (msg) {
    ...
    if (!msg._routingKey) {
        this.log.agent.info(
            'Error: message received without routingKey:\n'
            + inspect(msg));
            return;
    }
...
AMQPAgent.prototype.setupQueue = function (callback) {
    ...
    for (var i = 0, il = this.commandNames.length; i < il; i++) {
        self.queue.bind(
            'amq.topic',
            this.config.resource + '.' + this.commandNames[i] + '.' +
            this.uuid);
    }
